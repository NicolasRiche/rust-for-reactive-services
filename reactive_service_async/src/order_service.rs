...
pub trait EventsJournal<Event> {
    async fn persist_event(&mut self, entity_id: OrderId, evt_w_seq: &SequencedEvent<Event>) -> Result<(), &'static str>;
    async fn retrieve_events(&mut self, entity_id: OrderId) -> Result<Vec<SequencedEvent<Event>>, &'static str>;
}
    ...
    P: PaymentProcessor
{
    ...
    async fn create_and_process_entity_command<F>(&mut self, entity_id: OrderId, create_command: F)
    ...
    pub async fn update_cart(&mut self, cmd: UpdateCart)
        -> Result<(&OrderState, Vec<SequencedEvent<OrderEvent>>), &'static str> {
    ...
    pub async fn update_delivery_address(&mut self, cmd: UpdateDeliveryAddress)
       -> Result<(&OrderState, Vec<SequencedEvent<OrderEvent>>), &'static str> {
    ...
    pub async fn pay_order(&mut self, cmd: PayOrder)
       -> Result<(&OrderState, Vec<SequencedEvent<OrderEvent>>), &'static str> {
